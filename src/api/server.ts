...
function authenticateToken(req: Request, res: Response, next: any) {
  ...
  if (!token) {
    return res.status(401).json({
      success: false,
      error:
        'No token provided. Please include token in Authorization header, request body, or query parameter.',
    });
  }
  ...
  try {
    if (fs.existsSync(API_TOKEN_FILE)) {
      ...
      res.json({
        ...
        message:
          'Token exists. Check your data/api-token.json file or server logs for the actual token.',
      });
    } else {
      res.json({
        success: true,
        exists: false,
        message:
          'No token file found. Token will be generated on first API server start.',
      });
    }
  } catch (error) {
    res.status(500).json({
