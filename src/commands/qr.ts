...
export async function handleQrCommand(message: Message) {
  ...
  if (!text) {
    await message.reply('Please provide text to generate QR code. Usage: /qr <your text>');
    return;
  }

  if (text.length > 500) {
    await message.reply('Text is too long. Please keep it under 500 characters.');
    return;
  }
