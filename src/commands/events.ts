    ...
    if (fs.existsSync(EVENTS_DATA_FILE)) {
      ...
      Object.entries(eventsObj).forEach(([chatId, chatEvents]) => {
        if (chatEvents.activeEvent) {
          chatEvents.activeEvent.datetime = new Date(chatEvents.activeEvent.datetime);
          chatEvents.activeEvent.createdAt = new Date(chatEvents.activeEvent.createdAt);
          chatEvents.activeEvent.attendees = chatEvents.activeEvent.attendees.map(attendee => ({
            ...attendee,
      ...
      debugLog('Loaded events data for', Object.keys(eventsObj).length, 'chats');
    }
  } catch (error) {
...
async function isUserAdmin(message: Message): Promise<boolean> {
  try {
    ...
    for (const participant of participants) {
      debugLog('Checking participant:', participant.id._serialized, 'isAdmin:', participant.isAdmin);
...
export async function handleEventCreateCommand(message: Message) {
  try {
    ...
    try {
      ...
      debugLog('Error sending error reply in event create command:', replyError);
    }
  }
}
...
export async function handleEventCommand(message: Message) {
  try {
    ...
    const dismissedCount = event.attendees.filter(a => a.status === 'dismissed').length;
    ...
    try {
      await message.reply('Sorry, there was an error getting the event details.');
    } catch (replyError) {
...
export async function handleEventConfirmCommand(message: Message) {
  try {
    ...
    const existingAttendeeIndex = event.attendees.findIndex(a => a.phoneNumber === phoneNumber);
    ...
    if (existingAttendeeIndex >= 0) {
      ...
      await message.reply('✅ Your attendance has been updated to *confirmed*!');
    } else {
      // Add new attendee
      ...
      await message.reply('✅ Thank you! Your attendance has been *confirmed*!');
    }
    
    saveEventsData();
    debugLog('User confirmed attendance for event:', event.title, 'User:', name);
    
  } catch (error) {
    debugLog('Error in event confirm command:', error);
    try {
      await message.reply('Sorry, there was an error confirming your attendance.');
    } catch (replyError) {
      debugLog('Error sending error reply in event confirm command:', replyError);
    }
  }
}
...
export async function handleEventDismissCommand(message: Message) {
  try {
    ...
    saveEventsData();
    debugLog('User dismissed attendance for event:', event.title, 'User:', name);
    
  } catch (error) {
    debugLog('Error in event dismiss command:', error);
    try {
      await message.reply('Sorry, there was an error declining your attendance.');
    } catch (replyError) {
      debugLog('Error sending error reply in event dismiss command:', replyError);
    }
  }
}
...
export async function handleEventDeleteCommand(message: Message) {
  try {
    ...
    await message.reply(`🗑️ Event "*${eventTitle}*" has been deleted successfully.\n\nYou can now create a new event.`);
    ...
    try {
      ...
      debugLog('Error sending error reply in event delete command:', replyError);
    }
  }
}
